...
nelda3 <- nelda %>% dplyr::select(ccode, country, country_id, year, electionid, election_mdy)
nelda3 <- nelda3 %>% mutate(election_year = year)
nelda3 <- massmob %>% left_join(nelda3, by = c("country_id", "year"))
```
...
Excluding protest events in MM that take place more than 180 days before an election (events_pre) or more than 90 days after an election (events_post)

```{r}
elections_db <- nelda %>% dplyr::select(country_id, election_mdy) # A second dataframe with election dates by country

nelda3.sub <- slice(nelda3, 0)

for (i in 1:nrow(nelda3)) {
  country_id_temp <- nelda3$country_id[i]
  elections_db_temp <- elections_db %>% filter(country_id == country_id_temp)
  protest_event_slice <- nelda3[i,]
  for (j in 1:nrow(elections_db_temp)) {
    elections_db_temp <- elections_db_temp %>% mutate(days_to_election_all =  election_mdy - protest_event_slice$protest_mdy)
    #Negative numbers mean the protest is after the election, positive numbers mean the protest is before the election
    elections_db_temp <- elections_db_temp %>% mutate(days_to_election_all_num = as.numeric(days_to_election_all))
    elections_db_temp <- elections_db_temp %>% filter(days_to_election_all_num <= 180 & days_to_election_all_num >= -90)
  }
}
```
...
nelda3 <- nelda3 %>% group_by(country_id) %>% mutate(election_date_upcoming = as_date(ifelse(is.na(election_year)==F, election_mdy, 
                                                  ifelse(is.na(election_year)==T & is.na(lead(election_mdy))==F,
                                                                          lead(election_mdy), NA))))



nelda3 <- nelda3 %>% group_by(country_id) %>% mutate(election_date_lagged = as_date(ifelse(is.na(election_year)==F, election_mdy,                                                         ifelse(is.na(election_year)==T & is.na(lag(election_mdy))==F, lag(election_mdy), NA))))
