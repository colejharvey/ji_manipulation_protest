...
nelda3 <- massmob %>% left_join(nelda3, by = c("ccode", "year")) #Nelda3 is in progress, need to figure out a) if needed, b) how to include pre-election and post-election mobilization
```
...
size_data_mm_pre <- nelda3.sub.pre %>% group_by(electionid) %>% summarize(median.participants.pre = median(participants_category_ordered, na.rm = T))
size_data_mm_post <- nelda3.sub.post %>% group_by(electionid) %>% summarize(median.participants.post = median(participants_category_ordered, na.rm = T))
...
nelda2.sub <- nelda2.sub %>% left_join(mm_data_comb, by = "NeldaID")  ## Double check to make sure this joining works
