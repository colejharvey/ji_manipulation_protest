...
\toprule
                     &              &                           & \multicolumn{2}{l}{Strategic protest; manipulation = weakness} & \multicolumn{2}{l}{Non-strategic protest; manipulation = strength} \\ \midrule
Incumbent vote-share & Manipulation & True popularity inference & Initiation              & Prob. of pro-gov outcome             & Initiation                & Prob. of pro-gov. outcome              \\
Low                  & Low          & Low                       & Less likely             & More likely                          & Less likely               & Less likely                            \\
Low                  & High         & Very low                  & More likely             & Less likely                          & More likely               & More likely                            \\
Citations            &              &                           &                         &                                      &                           &                                        \\ \bottomrule
\end{tabular}
\caption{}
...
```{r theoryfig, echo=FALSE, fig.cap="A classic theory of election manipulation and protest", out.width="90%", fig.align='center'}
...
model.margin.binary <- glm(any_protest ~ lji.lag + legcon.1lag + gdpgro.1lag + altinf.1lag  +  + osorg.1lag  + natlpartyorg.lag + v2elirreg.inv +  +    winner.share.upd   + presidential + inc.termlimit.num  +  physinteg.1lag + physintlag.sq + urban.pct + rentierstate + taxstate + n.events.pre + unscheduled_election + regional_success_lag, family = binomial(link = "logit"), data = nelda2.sub %>% filter(inc.lose == "0"))
...
model.binary_noc <- glm(any_protest ~  v2elirreg.inv +  +    winner.share.upd*v2elirreg.inv  + presidential + inc.termlimit.num, family = binomial(link = "logit"), data = nelda2.sub %>% filter(inc.lose == "0"))
...
modelsummary(models2, output = "default",  stars = T, title = 'Table 3: Logit models of protest occurrence', coef_rename = 
               c("lji.lag" = "Judical independence (lag)", "legcon.1lag" = "Leg. constraints (lag)", 
                 ...
                 "physintlag.sq" = "Phys. int. lag squared", "regional_success_lag" = "Regional protest diffusion", "antisys.1lag" = "Anti-system CS activity", "winner.share.upd" = "Incumb. vote-share",
...
\toprule
                                                                                      & \begin{tabular}[c]{@{}l@{}}Successful \\ protests\end{tabular} & \begin{tabular}[c]{@{}l@{}}Failed \\ protests\end{tabular} & \begin{tabular}[c]{@{}l@{}}P-value \\ (successful vs. \\ unsuccessful)\end{tabular} & \begin{tabular}[c]{@{}l@{}}SD \\ (full sample)\end{tabular} \\ \midrule
\begin{tabular}[c]{@{}l@{}}Intent. \\ irregularities\end{tabular}                     & 1.55                                                           & 1.25                                                       & 0.26                                                                                & 0.93                                                        \\
Inc. vote-share                                                                       & 0.50                                                           & 0.59                                                       & 0.08                                                                                & 0.21                                                        \\
\begin{tabular}[c]{@{}l@{}}Pre-election \\ protests (n)\end{tabular}                  & 7.29                                                           & 6                                                          & 0.25                                                                                & 8.4                                                         \\
\begin{tabular}[c]{@{}l@{}}Pre-election protests\\ (median participants)\end{tabular} & 3.58                                                           & 3                                                          & 0.1                                                                                 & 0.89                                                        \\
\begin{tabular}[c]{@{}l@{}}Civil society \\ openness\end{tabular}                     & 0.30                                                           & 0.32                                                       & 0.85                                                                                & 1.08                                                        \\
Phys. int. rights                                                                     & 0.45                                                           & 0.43                                                       & 0.81                                                                                & 0.22                                                        \\ \bottomrule
\end{tabular}
...
Maybe add some thoughts on Kuran 1991--the idea being that authoritarian regimes are low info and rife with preference falsification. So opp has no choice but to gamble--so strategic in motivation but not in outcome or estimation?
