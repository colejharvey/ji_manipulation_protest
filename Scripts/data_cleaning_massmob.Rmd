...
nelda_massmob <- nelda_massmob %>% filter(is.na(protest_mdy) == T | protest_mdy < election_mdy | (protest_mdy >= election_mdy & str_detect(notes_massmob, regex(pattern = "election*|vot*|ballot*", ignore_case = T))))
```
...
```{r}
write.csv(nelda_complete, here::here("Data", "nelda_complete.csv"))
```