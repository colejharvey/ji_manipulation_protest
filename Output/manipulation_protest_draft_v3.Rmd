...
nelda3.sub <- nelda3.sub %>% mutate(protest.success = ifelse(inc.replaced.prot == 1 | elections.cancel.prot, 1, 0))
...
ggplot(nelda3.sub, aes(x=n.events.massmob.post)) + geom_histogram(bins = 90) + xlab("Number of protest events") + theme_bw()


ggplot(nelda3.sub, aes(x=winner.share.upd)) + geom_histogram(bins=100) + xlab("Incumbent vote-share") + theme_bw()
```

```{r, include=FALSE}


p.success <- ggplot(nelda3.sub, aes(x=as.numeric(median.participants.mm.post), fill = as.factor(protest.success), color=as.factor(protest.success)))+
...
```{r, include=FALSE}
mu <- plyr::ddply(nelda3.sub, "protest.success", summarise, grp.mean=mean(n.events.massmob.post))
head(mu)

p.success <- ggplot(nelda3.sub, aes(x=n.events.massmob.post, fill = as.factor(protest.success), color=as.factor(protest.success)))+
  geom_histogram(position="identity", alpha=0.5, binwidth = 1) + 
