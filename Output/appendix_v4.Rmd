...
Figure \@ref(fig:prot-rep-fraud) shows the absence of such an effect. In fact, there is no significant change in the predicted level of fraud at lower levels of physical integrity, even when pre-election protest is widespread. We do observe a significant effect when comparing high- and low-repression regimes in highly extreme cases\textemdash there are only three cases in the dataset with more than 70 protest events\textemdash and is in the opposite direction predicted by this counterargument. That is, more numerous pre-election protest is associated with higher predicted levels of fraud in less repressive states, not harsher ones. The figure also shows that less repressive regimes appear to respond to greater civic unrest with more fraud. Altogether, this robustness check shows that pre-election protest in a repressive environment does not appear to deter fraud, and supports the view that manipulation may be used to attempt to claim a large margin of victory or to prevent elite or bureaucratic splits in a challenging period.
...
#summary(m1_rep)
p.m1rep <- sjPlot::plot_model(m1_rep, type = "pred", terms = c("n.events.pre", "physinteg.1lag [.25, .63]"))
...
plot_binary_margin_succ_n <- sjPlot::plot_model(model.n.base2.succ.bin, type = "pred", terms = c("winner.share.upd [all]", "v2elirreg.inv [-.45, 1.78]")) #To set condition for successor value, use , condition = c(incumb.status = "Contested exec. election")
...
t6b <- huxreg(models5a, tidy_args = c("component" = "cond"), borders = 0, number_format = 2,  omit_coefs = c("sd__(Intercept)", "sd__Observation"), coefs = c("Intercept" = "(Intercept)", "Incumbent term-limited" = "inc.termlimit.num", "Election fraud" = "v2elirreg.inv", "Winner vote-share" = "winner.share.upd", "Election fraud : Winner vote-share" = "v2elirreg.inv:winner.share.upd"), statistics = character(0)) %>% set_caption("Models of number and size of protests, only electoral authoritarian regimes") %>% insert_row("", "Number", "Size", after = 0)
theme_compact(t6b, header_rows = TRUE, header_cols = FALSE)  %>% insert_row("Controls", "Yes", "Yes", after = 12) %>% set_bottom_border(12, everywhere)
```
...
p.n.margin.auth <- p.n.margin.auth + labs(x = "Incumbent vote-share", y = "Predicted number of protests" , title = "Predicted count of protest events", color = "Election fraud")  + theme_bw()
...
p.size.margin.auth <- p.size.margin.auth + labs(x = "Incumbent vote-share", y = "Predicted size of protests" , title = "Predicted maximum size of protest events", color = "Election fraud")  + theme_bw()
...
```{r n-protests-margin-auth, fig.cap="Election fraud, incumbent vote-share, and number of protest events in authoritarian regimes", echo=FALSE, out.width = '75%', fig.align='center', fig.pos="h!"}
p.n.margin.auth 

```

```{r size-protests-margin-auth, fig.cap="Election fraud, incumbent vote-share, and maximum size of protest events in authoritarian regimes", echo=FALSE, out.width = '75%', fig.align='center', fig.pos="h!"}
p.size.margin.auth 
...
model.n.base2.elfr <- glmmTMB(n.events.post ~ jucon.1lag + legcon.1lag + altinf.1lag + libdem.1lag + gdpgro.1lag +  + osorg.1lag  + natlpartyorg.lag +  v2xel_frefair*winner.share.upd   + presidential + inc.termlimit.num  +  physinteg.1lag + physintlag.sq + urban.pct + rentierstate + taxstate + n.events.pre + unscheduled_election + regional_success_lag,
                               ziformula=~.,
                              data = nelda2.sub %>% filter(v2x_regime <= 2 & inc.lose == "0"),
                                family=truncated_nbinom2)
...
p.n.margin <- plot(dat)
p.n.margin <- p.n.margin + labs(x = "Incumbent vote-share", y = "Predicted number of protests" , title = "Predicted count of protest events",  color = "Election integrity")  + theme_bw() + coord_cartesian(ylim=c(0, 75))
...
p.size.margin <- p.size.margin + labs(x = "Incumbent vote-share", y = "Predicted size of protests" , title = "Predicted maximum size of protest events", color = "Election integrity")  + theme_bw() + coord_cartesian(ylim = c(0, 50000))
...
```{r n-protests-margin-fulldv, fig.cap="Election integrity, incumbent vote-share, and number of protest events", echo=FALSE, out.width = '75%', fig.align='center', fig.pos="h!"}
p.n.margin 

```

```{r protests-size-fulldv, fig.cap="Election integrity, incumbent vote-share, and size of protest events", echo=FALSE, out.width = '75%', fig.align='center', fig.pos="h!"}
p.size.margin 
